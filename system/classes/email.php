...
class Email
{
	...
	public $_priorities    = array(1 => '1 (Highest)',2 => '2 (High)',3 => '3 (Normal)',4 => '4 (Low)',5 => '5 (Lowest)');

	public function __construct($config = array())
	{
		if(empty($config))
		{
			$config = conf('email',array());
		}
		else
		{
			$config = array_merge(conf('email',array()),$config);
		}
	...
	public function send($auto_clear = TRUE)
	{
		if ( ! isset($this->_headers['From']))
		{
			if($this->_get_protocol() == 'smtp')
			{
				$this->from($this->smtp_user,$this->smtp_name);
			}
			else
			{
	...
	protected function _set_error_message($msg, $val = '')
	{
		...
		if (substr($msg, 0, 5) != 'lang:' || FALSE === ($line = lang(substr($msg, 5))))
		{
