...
if (!function_exists('xml_parser_create')) message('Your PHP installation does not support XML');
	...
	function decode($array=FALSE)
	{
		...
		if ($array !== FALSE && is_array($array))
		{
			while (list($key) = each($array))
			{
				...
				{
					$array[$key] = ($this->xss_clean) ? $obj->xss_clean($array[$key]) : $array[$key];
				}
			}
		...
		else
		{
			...
			{
				$result = ($this->xss_clean) ? $obj->xss_clean($result) : $result;
			}
		}
	...
	function output_parameters($array=FALSE)
	{
		...
		if ($array !== FALSE && is_array($array))
		{
			while (list($key) = each($array))
			{
				...
				else
				{
					...
					$array[$key] = ($key == 'bits' OR $this->xss_clean == FALSE) ? $array[$key] : $obj->xss_clean($array[$key]);
				}
			}
		...
		else
		{
			...
			for ($i = 0; $i < count($this->params); $i++)
			{
				...
				{
					$parameters[] = ($this->xss_clean) ? $obj->xss_clean($a_param) : $a_param;
				}
			}
		}
